...
// helpers
 ...
 // 这对选项需要一起使用，以允许一个祖先组件向其所有子孙后代注入一个依赖，不论组件层次有多深，并在起上下游关系成立的时间里始终生效。如果你熟悉 React，这与 React 的上下文特性很相似。
    ...
    function mergeOptions(parent, //父值
                          child, //子值 优选取子值
                          vm) {
        debugger // Ethan: debugger 检查当前实例components是否规范（正则匹配大小写和横岗）
        {
            //检验子组件
        ...
        debugger // Ethan: debugger 检查当前实例Props是否规范(是否是对象或数组)
        //检查 props 数据类型
       ...
       // * normalizeDirectives获取到指令对象值。循环对象指令的值，如果是函数则把它变成dirs[key] = {bind: def, update: def} 这种形式
        ...
        debugger // Ethan: debugger 把当前实例的mixins里的配置项合并到parent实例的配置项里
        if (child.mixins) {
        ...
        var options = {};
        var key;
        debugger // Ethan: 将parent和当前配置的options进行合并，如果childVal不存在，就返回parentVal,如果存在，就把childVal添加到parentVal后，比如生命钩子
        ...
        //返回参数
        debugger // Ethan: debugger 看一下options
        return options
    }
...
//createAsyncPlaceholder 创建简单的占位符 创建一个节点
    ...
    function mountComponent(
        ...
        /* istanbul ignore if */
        //如果开发环境
        debugger // Ethan: debugger 定义updateComponent方法，传给页面P的watcher里去执行，先执行vm._render()方法获取最新的节点，再执行vm._update()比较最新和就的节点（即patch），完成渲染
        ...
        //创建观察者
        debugger // Ethan: debugger 创建watcher，只要页面中有读取数据，数据就会存取这个watcher，假如数据修改，就会触发wather.update，然后触发updateComponent方法，传给页面P的watcher里去执行，先执行vm._render()方法获取最新的节点，再执行vm._update()比较最新和就的节点（即patch），完成渲染
        new Watcher(
            vm,  //vm vode
    ...
    function initState(vm) {
        debugger //初始化观察者队列，存储所有的watcher
        vm._watchers = []; 
...
//类在槽节点上使
    ...
    function initMixin(Vue) {
        Vue.prototype._init = function (options) { //初始化函数
            ...
            {
                //初始化 代理 监听
                debugger // Ethan: debugger 只是警告代理
                initProxy(vm);
            }
            // expose real self 暴露真实的self
            vm._self = vm;
            debugger // Ethan: debugger 给当前实例挂载父组件放入$parent，并把找到的父组件挂载当前组件放入$children
            initLifecycle(vm); //初始化生命周期 标志
            initEvents(vm); //初始化事件
            initRender(vm); // 初始化渲染
            debugger // Ethan: debugger beforeCreate的$el和$data都没有
            ...
            debugger // Ethan: debugger created开始执行
    ...
    function resolveConstructorOptions(Ctor) {
        debugger // Ethan:debugger options是Vue构造函数的options，是一些全局的配置，比如全局组件，全局自定义指令等
        var options = Ctor.options;
...
// public mount method 安装方法 实例方法挂载 vm
    ...
    Vue.prototype.$mount = function (el,  //真实dom 或者是string
        ...
        debugger // Ethan: debugger 进入挂载阶段
...
// #6828: chrome encodes content in a[href]
    ...
    Vue.prototype.$mount = function (el, hydrating) { //重写Vue.prototype.$mount
        ...
        if (!options.render) {
            ...
            debugger // Ethan: debugger 判断有无template,没有的话直接获取el做template
            if (template) { //如果有模板
                debugger // Ethan: debugger template必须是id字符串或者是dom元素，不然就拿el作为innerHtml
            ...
            if (template) {
                ...
                console.log(options.comments)
                debugger // Ethan: debugger 将模板字符串转为render函数
               ...
               */
              debugger // Ethan: debugger 将render函数传给$options
        ...
        debugger // Ethan: debugger 调用$mount方法
