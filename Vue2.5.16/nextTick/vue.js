...
// helpers
 ...
 // 这对选项需要一起使用，以允许一个祖先组件向其所有子孙后代注入一个依赖，不论组件层次有多深，并在起上下游关系成立的时间里始终生效。如果你熟悉 React，这与 React 的上下文特性很相似。
    ...
    function flushCallbacks() {
        debugger // Ethan: debugger 开始触发callbacks的函数
...
// Determine microtask defer implementation.
    ...
    if (typeof Promise !== 'undefined' && isNative(Promise)) {
        ...
        microTimerFunc = function () {
            debugger // Ethan: debugger flushCallbacks遍历callbacks执行callback
    ...
    function nextTick(cb, ctx) {
        ...
        debugger // Ethan: debugger callbacks是flushSchedulerQueue和开发者回调函数的集合
        ...
        if (!pending) {
            debugger // Ethan: debugger pending为false就要执行触发callbacks 队列函数
            pending = true;
            //执行异步宏任务
            if (useMacroTask) {
...
//createAsyncPlaceholder 创建简单的占位符 创建一个节点
    ...
    function queueWatcher(watcher) {
        debugger // Ethan: debugger
        var id = watcher.id;
        debugger // Ethan: debugger 第二次修改的时候就直接跳过了，因为has[id]只有在微任务才会重置为null，此时宏任务没有走完，微任务还不能开始执行
        if (has[id] == null) {
            has[id] = true;
            ...
            if (!flushing) {
                debugger // Ethan: debugger queue用于最后微任务中遍历queue，完成更新
            ...
            if (!waiting) {
                waiting = true;
                //为callbacks 收集队列cb 函数 并且根据 pending 状态是否要触发callbacks 队列函数
                debugger // Ethan: debugger flushSchedulerQueue相当于queue封装的函数，注意：函数也包含开发者使用的nextTick
                nextTick(
    ...
    Watcher.prototype.update = function update() {
        ...
        if (this.lazy) { //懒惰的 忽略
            ...
            debugger // Ethan: debugger 页面P引用了数据D，修改了D，触发P的watcher
            //如果是多个观察者
