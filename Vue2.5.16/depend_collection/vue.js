...
//他们的榆树参考。
    ...
    var Observer = function Observer(value) {
        ...
        this.value = value;
        debugger // Ethan: debugger 赋值给obj
        this.dep = new Dep();
        this.vmCount = 0;
        //设置监听 value 必须是对象
        debugger // Ethan: debugger 将Observer实例绑定到data的__ob__
    ...
    Observer.prototype.walk = function walk(obj) {
        ...
        for (var i = 0; i < keys.length; i++) {
            debugger // Ethan: debugger 遍历data中的每一个属性
...
// helpers
    ...
    function observe(value, asRootData) {
        ...
        var ob;
        debugger // Ethan: debugger 数据劫持过的直接返回
        if (hasOwn(value, '__ob__') && value.__ob__ instanceof Observer) {
            ...
            debugger // Ethan: debugger 开始数据劫持
    ...
    function defineReactive(obj, //对象
        ...
        debugger // Ethan: debugger 实例化依赖，闭包，get的时候会用到
        var dep = new Dep();
        //获取描述属性
        ...
        debugger // Ethan: debugger 递归
        var childOb = !shallow && observe(val);
        //定义描述
        debugger // Ethan: debugger 开始定义响应式
        Object.defineProperty(obj, key, {
            enumerable: true,
            configurable: true,
            get: function reactiveGetter() {
                ...
                if (Dep.target) {  //Dep.target 静态标志 标志了Dep添加了Watcher 实例化的对象
                    debugger // Ethan: debugger Watcher 实例化的对象
                    //添加一个dep
                    dep.depend();
    ...
    function mergeDataOrFn(
                           ...
                           vm
                                    ...
                                    if (!vm) {
                                        ...
                                        debugger // Ethan: debugger 返回一个新的data函数
                                                        //如果vm 存在 则是合并vm的数据
                                                        return function mergedInstanceDataFn() {
                                                            debugger // Ethan: debugger 执行mixins的data和child的data合并
                                                                        // instance merge
    ...
    strats.data = function (
                            ...
                            vm
                                    ...
                                    debugger // Ethan: debugger 将mixins的data和child的data执行后再合并
                                if (!vm) {
    ...
    function mergeOptions(parent, //父值
       ...
       // * normalizeDirectives获取到指令对象值。循环对象指令的值，如果是函数则把它变成dirs[key] = {bind: def, update: def} 这种形式
        ...
        for (key in child) {  //循环子组件的
            debugger // Ethan: debugger key为data的时候进入,否则跳出
...
//createAsyncPlaceholder 创建简单的占位符 创建一个节点
    ...
    function initData(vm) {
        ...
        debugger // Ethan: debugger 判断data的属性和props和methods不能重复
        while (i--) { //循环data
            var key = keys[i];
            {
        ...
        console.log('data')
        console.log(data)
        debugger // Ethan: debugger 数据劫持
        observe(data, true /* asRootData */);
    }

    //转换数据 如果数据是 一个函数的时候 执行该函数 拿到数据
    function getData(data, vm) {
        debugger // Ethan: debugger 执行新data函数
        // #7573 disable dep collection when invoking data getters
        //调用数据getter时禁用dep收集
        pushTarget();
        try {
            //执行函数 获取数据
            debugger // Ethan: debugger 执行新的data函数，返回对象
