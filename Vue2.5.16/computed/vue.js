...
//createAsyncPlaceholder 创建简单的占位符 创建一个节点
    ...
    var Watcher = function Watcher(
        ...
        //把观察者添加到队列里面 当前Watcher添加到vue实例上
        debugger // Ethan: debugger 将wathcher实例绑定到_watchers，所以vm._watchers里有watch和computed的观察者
        vm._watchers.push(this);
        // options
        if (options) { //如果有参数
        ...
        debugger // Ethan: debugger 将回调函数赋值给watch.cb，当监听到变量变化时，变量的set会触发watch.cb()
        this.cb = cb; //回调函数
        this.id = ++uid$1; // uid for batching uid为批处理  监听者id
        this.active = true; //激活
        debugger // Ethan: debugger computed的this.dirty===true表示不会马上读取值
        ...
        if (typeof expOrFn === 'function') {
            //获取值的函数
            debugger // Ethan: debugger computed保存给getter
            this.getter = expOrFn;
        } else {
            ...
            debugger // Ethan: debugger 得到根据路径解析获取data变量的函数
            this.getter = parsePath(expOrFn);
            console.log('Ethan: ', this.getter)
        ...
        debugger // Ethan: debugger 和watch不一样，computed不会直接读取值, 所以value为undefined
    ...
    Watcher.prototype.get = function get() {
        ...
        var vm = this.vm;
        try {
            debugger // Ethan: debugger 根据watch设置的key查找data中的值，之前data中的每个数据都应该有响应式了，所以获取了值，依赖收集器就会收集watch-watcher 
        ...
        //返回值
        debugger // Ethan: debugger 将获取到的值直接返回给watcher.value
        return value
    };
    ...
    Watcher.prototype.update = function update() {
        ...
        if (this.lazy) { //懒惰的 忽略
            debugger // Ethan: debugger computed里关联的data数据改变，watcher.dirty变为true，表示是脏值
            this.dirty = true;
    ...
    function initState(vm) {
        ...
        if (opts.computed) { //计算属性
            debugger // Ethan: debugger computed开始
            //初始化计算属性 并且判断属性的key 是否 在 data ，将 计算属性的key 添加入监听者中
            initComputed(vm, opts.computed);
        }
        //options 中的 watch
        if (opts.watch && opts.watch !== nativeWatch) {
            debugger //Ethan: debugger 初始化Watch
    ...
    function initComputed(vm, computed) {
        // $flow-disable-line
        //创建一个新的监听者对象空对象
        debugger // Ethan: debugger 所有的computed的watcher都保存在_computedWatchers
        ...
        for (var key in computed) {
            ...
            if (!isSSR) { //如果不是node ssr渲染
                ...
                debugger // Ethan: debugger 和watch一样，创建观察者
            ...
            if (!(key in vm)) { //如果computed 属性key 不在虚拟dom中
                debugger // Ethan: debugger 对computed里的key值进行对象监听
    ...
    function defineComputed(target, //目标
        ...
        debugger // Ethan: debugger 判断computed是否为对象还是function，判断有无缓存，无缓存执行createComputedGetter(key)，并设置set
        ...
        //添加对象监听
        debugger // Ethan: debugger 给vm直接添加computed对象监听，computed初始化结束
        Object.defineProperty(target, key, sharedPropertyDefinition);
    }

    //创建计算属性 获取值 收集 dep 依赖
    function createComputedGetter(key) {
        debugger // Ethan: debugger 给get设置一个新函数
        return function computedGetter() {
            // Watcher 实例化之后的对象
            debugger // Ethan: debugger 当读取computed值的时候触发
            var watcher = this._computedWatchers && this._computedWatchers[key];
            if (watcher) {
                if (watcher.dirty) {
                    //this.value 获取值 this.getter
                    debugger // Ethan: debugger 脏值为true表示缓存已脏，重新调用watcher.get(),赋值给this.value, this.dirty = false
                    watcher.evaluate(); //评估
                }
                if (Dep.target) {
                    //为Watcher 添加 为Watcher.newDeps.push(dep); 一个dep对象
    ...
    function initWatch(vm, watch) {
        //循环watch对象
        debugger // Ethan: 遍历watch里的每一个属性，执行createWatcher，带上参数
        for (var key in watch) {
            ...
            if (Array.isArray(handler)) {
                ...
                //循环数组 创建 监听
                debugger // Ethan: debugger 处理handler，变成真正的函数
                createWatcher(
                    vm, // vm 是 vue对象
                    key, //key
                    handler //函数或者对象
    ...
    function createWatcher(vm,  //vm对象
        ...
        debugger // Ethan: createWatcher的作用：原始属性的value给options, handler变成真正的函数, 判断watch里的value，是对象，获取对象的handler属性，是字符串，获取vm上的函数
    ...
    function stateMixin(Vue) {
        ...
        Vue.prototype.$watch = function (expOrFn, //用户手动监听
            ...
            //实例化Watcher 观察者
            debugger // Ethan: 实例化Watcher 观察者
            console.log('Ethan: expOrFn, cb, options', expOrFn, cb, options)
            ...
            if (options.immediate) {
                debugger // Ethan： watch中的属性中有immediate参数，立即执行，所以是在beforeCreate和created之间执行
                //回调触发函数
...
//类在槽节点上使
    ...
    function initMixin(Vue) {
        Vue.prototype._init = function (options) { //初始化函数
            debugger  // Ethan: debugger 开始初始化$options
            var vm = this;
            // a uid
            vm._uid = uid$3++; //id
            ...
            debugger // Ethan：debugger 处理 data，props，computed 等数据，进入
            initState(vm);  //    //初始化状态
    ...
    function Vue(options) {
        ...
        debugger // Ethan: debugger 初始化options，进入
        this._init(options);
    }
